...
export function NamePlate({ id}) {
  const nameplate = [{ value: "Asset Name", units: "", sym: "assetName" },
   { value: "MVA", units: "MVA", sym: "MVA_Rating" },
    { value: "AMPERES", units: "A", sym: "ampereRating" }, 
    { value: "LOAD VOLTAGE", units: "KV", sym: "VoltageRating" }, 
    { value: "Area of Cross section ", units: "mm2", sym: "crossSectionArea" }];
  const [rating, setRating] = useState(undefined);
  function sendRating() {
    axios.post(`${API}:${9000}/cable/nameplate?id=${id}`, rating);
  }

  useEffect(() => {
    axios.get(`${API}:${9000}/cable/nameplate?id=${id}`)
      .then((data) => {
        setRating(data.data);
      });
  }, []);
